  ...
  return (
    <header
      className={`sticky top-0 z-50 border-b transition-shadow ${
        scrolled
          ? "border-line bg-paper/95 shadow-[0_1px_24px_rgba(26,25,21,0.08)] backdrop-blur"
    ...
    >
      ...
      {open ? (
        <nav
          ...
          className="border-t border-line bg-paper px-4 pb-6 pt-2 lg:hidden"
        >
                ...
                <li key={item.href}>
                  <Link
                    ...
                    className={`block rounded-xl px-3 py-3 text-lg font-medium ${
                      active ? "bg-paper-2 text-ink" : "text-ink-soft"
                    }`}
                  >
                    {item.label}
                  </Link>
